...
import { AlertCircle, CheckCircle, Clock, Package, Search, TrendingUp, User } from "lucide-react";
...
export default function DashboardAdmin() {
  ...
  const stats = useMemo(() => {
    ...
    const totalUsers = users?.filter((admin: Users) => admin.role === "ADMIN").length || 0;

    // Count claimed/returned items
    const claimed = foundReports?.filter((item: FoundReport | undefined) => item?.statusReport === "Done" || item?.statusReport === "Closed").length || 0;
  ...
  const recentItems = useMemo(() => {
    ...
    const foundItems: RecentItem[] = (foundReports || []).map((item: FoundReport) => ({
      ...item,
      type: "ditemukan" as const,
    }));

    const lostItems: RecentItem[] = (lostReports || []).map((item: LostReport) => ({
      ...item,
                ...
                <section className="p-6">
                  ...
                  <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    {statsConfig.map((stat, index) => {
                      ...
                      return (
                        <section key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                  ...
                  <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <section className="divide-y divide-gray-200">
                        {recentItems.length === 0 ? (
                          ...
                          recentItems.map((item, index) => (
                            <section key={index} className="p-6 hover:bg-gray-50 transition-colors">
                              <section className="flex items-center justify-between">
                                <section className="flex items-center gap-4">
                                  <section className={`p-2 rounded-lg ${item.type === "hilang" ? "bg-red-50" : "bg-blue-50"}`}>
                                    {item.type === "hilang" ? <Search className="w-5 h-5 text-red-600" /> : <Package className="w-5 h-5 text-blue-600" />}
                                  </section>
                                  <section>
                                    <p className="font-medium text-gray-900">{item.itemName || "Barang"}</p>
                                    <section className="flex items-center gap-2 mt-1">
                                      <span className={`text-xs px-2 py-1 rounded-full ${item.type === "hilang" ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"}`}>
                                        {item.type === "hilang" ? "Barang Hilang" : "Barang Ditemukan"}
                                      </span>
                                      {item.status && (
                                        <span
                                          className={`text-xs px-2 py-1 rounded-full ${
                                            item.status === "PENDING" ? "bg-yellow-100 text-yellow-700" : item.status === "APPROVED" || item.status === "REJECTED" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"
                                          }`}
                                        >
                                          {item.status === "PENDING" ? "PENDING" : item.status === "APPROVED" ? "Diklaim" : item.status === "REJECTED" ? "Dikembalikan" : item.status}
                                        </span>
                                      )}
                                    </section>
                                  </section>
                                ...
                                <section className="flex items-center gap-2 text-gray-500 text-sm">
                                  ...
                                  {formatTimeAgo(item.createdAt || item.createdAt || "")}
                                </section>
                              </section>
                            </section>
                          ))
                    ...
                    <section className="space-y-6">
                      ...
                      <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Tingkat Keberhasilan</h3>
                        <section className="flex items-center justify-center">
                          <section className="relative w-32 h-32">
                            <svg className="w-32 h-32 transform -rotate-90">
                              <circle cx="64" cy="64" r="56" stroke="#e5e7eb" strokeWidth="12" fill="none" />
                              <circle cx="64" cy="64" r="56" stroke="#10b981" strokeWidth="12" fill="none" strokeDasharray={`${2 * Math.PI * 56 * (successRate / 100)} ${2 * Math.PI * 56}`} strokeLinecap="round" />
                            </svg>
                        ...
                        <p className="text-center text-sm text-gray-600 mt-4">
                          {stats.claimed} dari {stats.totalFound} barang berhasil dikembalikan
                        </p>
                      </section>
                      ...
                      <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        ...
                        <section className="space-y-3">
                          <Button className="w-full bg-blue-500 hover:bg-blue-600 text-white font-medium py-3 px-4 rounded-lg transition-colors" onClick={() => router.push("/homepage/admin/found")}>
                            Lihat Laporan Temuan
                          </Button>
                          <Button className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-3 px-4 rounded-lg transition-colors" onClick={() => router.push("/homepage/admin/list-user")}>
                            Kelola User
                          </Button>
                        </section>
                      </section>
